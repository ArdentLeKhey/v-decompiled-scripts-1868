...
int func_18(var uParam0, int iParam1)
{
	...
	if (uParam0->f_11)
	{
		...
		if (!func_25(44))
		{
			...
			{
				if (MISC::GET_DISTANCE_BETWEEN_COORDS(func_23(iVar4), vVar3, false) > 200f)
				{
...
int func_28(int iParam0)
{
	...
	if (iParam0 == 2)
	{
		...
		{
			if (MISC::IS_BIT_SET(Global_111560.f_10044.f_108, func_33(func_34(iParam0), iVar1)))
			{
				iVar0++;
			}
			iVar1++;
		...
		case 1:
			...
			if (!func_62())
			{
				if (func_50(iLocal_64))
				{
					...
					VEHICLE::SET_VEHICLE_ENGINE_ON(iLocal_64, false, true, 0);
...
int func_62()
{
	if (func_50(PLAYER::PLAYER_PED_ID()))
	{
		...
		else if (func_63())
		{
			...
			{
				ENTITY::SET_ENTITY_AS_MISSION_ENTITY(iLocal_64, true, 1);
...
void func_64()
{
	...
	if (func_24(&Local_42, &Local_48, vVar0, &iLocal_51))
	{
		...
		fVar2 = MISC::GET_DISTANCE_BETWEEN_COORDS(vVar1, vVar0, false);
		if (fVar2 > 200f)
		{
			func_66();
			if (!iLocal_61)
			...
			if (func_63())
			{
				...
				if (!HUD::DOES_BLIP_EXIST(iLocal_53))
				{
					...
					HUD::SET_BLIP_HIDDEN_ON_LEGEND(iLocal_53, 0);
			...
			if (Local_42.f_1.f_4 < Local_42.f_1.f_3)
			{
				...
				else
				{
					...
					if (!HUD::DOES_BLIP_EXIST(iLocal_55))
					{
						...
						HUD::SET_RADIUS_BLIP_EDGE(iLocal_55, 1);
					...
					if (func_63())
					{
						...
						while (iVar3 <= 3)
						{
							if (!HUD::DOES_BLIP_EXIST(iLocal_56[iVar3]))
							{
								...
								HUD::SET_RADIUS_BLIP_EDGE(iLocal_56[iVar3], 1);
